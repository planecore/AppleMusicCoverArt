...
async function generate() {
  ...
  setTimeout(async () => {
    ...
    await capctureImage()
    await capctureImage()
    const blob = await capctureImage()
...
async function capctureImage() {
  const scale = 750 / art.offsetWidth
  return await domtoimage.toBlob(art, {
    height: art.offsetHeight * scale,
    width: art.offsetWidth * scale,
    style: {
      transform: "scale(" + scale + ")",
      transformOrigin: "top left",
      width: art.offsetWidth + "px",
      height: art.offsetHeight + "px",
    },
  })
}
...
function fading(transition) {
  ...
  let fade = () => {
    ...
    delay(
      `artDiv.style.opacity = '${opacity}';artDiv.style.transform = 'scale(${scale})'`
    )
    for (let i = 0; i < $.querySelectorAll(".box").length; i++) {
      delay(
        `$.querySelectorAll('.box')[${i}].style.opacity = '${opacity}';$.querySelectorAll('.box')[${i}].style.transform = 'scale(${scale})'`
      )
    }
    for (let i = 0; i < $.querySelectorAll(".input").length; i++) {
      delay(
        `$.querySelectorAll('.input')[${i}].style.opacity = '${opacity}';$.querySelectorAll('.input')[${i}].style.transform = 'scale(${scale})'`
      )
    }
    for (let i = 0; i < $.querySelectorAll(".button").length; i++) {
      delay(
        `$.querySelectorAll('.button')[${i}].style.opacity = '${opacity}';$.querySelectorAll('.button')[${i}].style.transform = 'scale(${scale})'`
      )
    }
  }
...
function randomImage() {
  $.querySelectorAll(".image")[
    Math.floor(Math.random() * ($.querySelectorAll(".image").length - 0) + 0)
  ].click()
}
...
$.getElementById("file-input").addEventListener("change", (event) => {
  const file = event.target.files[0]
  setAlbum(URL.createObjectURL(file))
})
